...
//  LED WS2812
// ─────────────────────────────────────────────────────────
void updateLED(bool reminderActive = false) {
  int reminderBox = -1;
  if (reminderActive) {
    for (int i = 0; i < NUM_SENSORS; i++) {
      if (prevState[i]) { reminderBox = i; break; }
    }
  }
  for (int i = 0; i < NUM_SENSORS; i++) {
    if (!prevState[i])
      strip.setPixelColor(i, strip.Color(255, 0,   0));
    else if (i == reminderBox)
...
void checkReminder() {
  ...
  for (int i = 0; i < schedules[todayIdx].numTimes; i++) {
    if (schedules[todayIdx].times[i].equals(String(currentTime))) {
      Serial.printf("[REMINDER] Uong thuoc luc %s\n", currentTime);
